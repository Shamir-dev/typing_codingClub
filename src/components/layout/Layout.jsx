...
export default function Layout() {
  ...
  return (
    <div className="h-screen w-screen flex bg-base text-text overflow-hidden">
      <Sidebar
      ...
      <main className="flex-1 h-full min-w-0 overflow-hidden flex flex-col">
        <header className="relative h-20 shrink-0 px-5 sm:px-8 flex items-center justify-end gap-3">
          <button
            onClick={() => setShowStreaks((value) => !value)}
            aria-expanded={showStreaks}
            className="hidden sm:flex items-center gap-2 rounded-xl border border-line bg-panel-raised/70 px-3 py-2 text-left shadow-sm hover:border-accent-purple/60 transition-colors"
          >
          ...
          {showStreaks && (
            <div className="absolute right-28 top-[68px] z-30 w-72 rounded-xl border border-line bg-panel p-4 shadow-2xl animate-pop-in">
          ...
          <button title="Notifications" className="relative grid h-10 w-10 place-items-center rounded-xl border border-line bg-panel-raised/70 text-text-muted hover:text-text transition-colors">
